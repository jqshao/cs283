...
int get_student(int fd, int id, student_t *s)
{
    ...
    off_t trySeek = lseek(fd, offset, SEEK_SET);
    if (trySeek == -1)
    {
        return ERR_DB_FILE;
    }

    student_t buffer[STUDENT_RECORD_SIZE];
    ssize_t tryRead = read(fd, buffer, STUDENT_RECORD_SIZE);
    if (tryRead == -1)
    {
        return ERR_DB_FILE;
    }

    if (tryRead < STUDENT_RECORD_SIZE && tryRead != 0)
    {
        return ERR_DB_FILE;
    }

    student_t emptyBuffer[64] = {0};
    ...
    s->id = buffer->id;
    strncpy(s->fname, buffer->fname, sizeof(s->fname));
    strncpy(s->lname, buffer->lname, sizeof(s->lname));
    s->gpa = buffer->gpa;
    return NO_ERROR;
}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    ssize_t tryWrite = write(fd, &student, STUDENT_RECORD_SIZE);
    if (tryWrite == -1)
    {
...
int del_student(int fd, int id)
{
    ...
    else
    {
        ...
        ssize_t tryWrite = write(fd, &emptyStudent, STUDENT_RECORD_SIZE);
        if (tryWrite == -1)
        {
            printf(M_ERR_DB_WRITE);
            return ERR_DB_FILE;
...
int count_db_records(int fd)
{
    ...
    off_t trySeek = lseek(fd, STUDENT_RECORD_SIZE, SEEK_SET);
    if (trySeek == -1)
    {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    ...
    while (tryRead != 0)
    {
        ...
        tryRead = read(fd, buffer, STUDENT_RECORD_SIZE);
    }
    ...
    {
        printf(M_DB_RECORD_CNT, numOfStudents);
    }
...
int print_db(int fd)
{
    ...
    while (tryRead != 0)
    {
        ...
        if (memcmp(&buffer, &emptyBuffer, STUDENT_RECORD_SIZE) != 0)
        {
            ...
            float calculated_gpa_from_student = buffer->gpa / 100.0;
            printf(STUDENT_PRINT_FMT_STRING, buffer->id, buffer->fname, buffer->lname, calculated_gpa_from_student);
        }
