...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1)
    {
        ...
        if (strlen(cmd_buff) >= RDSH_COMM_BUFF_SZ)
        {
        ...
        while ((recv_size = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ, 0)) > 0)
        {
            ...
            rsp_buff[recv_size] = '\0'; // null terminate the buffer

            // At this point we have some data, lets see if this is the last chunk
            is_last_chunk = ((char)rsp_buff[recv_size - 1] == eof_char) ? 1 : 0;
        ...
        if (strcmp(rsp_buff, "exiting client connection") == 0 ||
            strcmp(rsp_buff, "stopping the server") == 0)
        {
            break;
        }
    }
