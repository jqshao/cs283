...
int boot_server(char *ifaces, int port)
{
    ...
    if (inet_pton(AF_INET, ifaces, &addr.sin_addr.s_addr) <= 0)
    {
        perror("inet_pton");
        return ERR_RDSH_COMMUNICATION;
    }
...
int exec_client_requests(int cli_socket)
{
    ...
    while (1)
    {
        ...
        char *eof_pos = strchr(io_buff, RDSH_EOF_CHAR);
        if (eof_pos)
        {
            *eof_pos = '\0';
        }
        ...
        cmd_rc = rsh_match_command(cmd_list->commands[0].argv[0]);
        if (cmd_rc == BI_NOT_BI)
        {
        ...
        else
        {
            ...
            {
                // Child process
                if (dup2(cli_socket, STDOUT_FILENO) == -1 || dup2(cli_socket, STDERR_FILENO) == -1)
                {
                    perror("dup2");
                    exit(EXIT_FAILURE);
                }

                last_rc = exec_built_in_cmd(&cmd_list->commands[0]);
                exit(last_rc);
            }
            else if (pid < 0)
            {
...
int send_message_string(int cli_socket, char *buff)
{
    ...
    int buff_len = snprintf(send_buff, RDSH_COMM_BUFF_SZ, "%s", buff);

    if (send(cli_socket, send_buff, buff_len + 1, 0) == -1)
    {
        perror("send");
        free(send_buff);
        return ERR_RDSH_COMMUNICATION;
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist)
{
    ...
    for (int i = 0; i < clist->num; i++)
    {
        ...
        if (pids[i] == 0)
        {
            // Redirect stdin for first command
            if (i == 0)
            {
            ...
            // Redirect stdout for last command
            if (i == clist->num - 1)
            {
